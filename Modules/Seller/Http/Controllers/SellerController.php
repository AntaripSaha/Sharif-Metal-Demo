...
class SellerController extends BaseController
{
    ...
    // Updating Request Products Table with price and Production Price End
        ...
        if ($request->isMethod('post')) {
            ...
            $sell_reqs = SellRequest::get();

            // foreach( $sell_reqs as $sell_req ){
            //     $sell_req->req_ids = $sell_req->req_id;
            //     $sell_req->save();
            // }
            // return "Success";

            return view('seller::new_sellreq', compact('customers', 'products', 'companies', 'sellers', 'role_id'));
            exit;
        }
    }
    ...
    public function edit_requisition_details(Request $request, $id)
    {
        if ($request->isMethod('post')) {
            ...
            return view('seller::edit_sale_requisition', compact(
                'sale_request_master',
                'product_details',
                'customer_name',
                'seller_info',
    ...
    public function sales_discount_edit(Request $request, $id)
    {
        ...
        foreach ($req_products as $key => $req_product) {
            ...
            $single_product_price =  $req_product->del_qnt * $req_product->products->price;
    ...
    public function add_collection(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $companies = Company::where('parent_id', '!=', 0)->get();
            return view('seller::collection', compact('customers', 'companies'));
            exit;
        }
    }
    /*Print Invoice*/
    ...
    public function direct_sale(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $companies = Company::where('parent_id', '!=', 0)->get();
            return view('seller::direct_sale', compact('customers', 'products', 'companies', 'warehouses'));
            exit;
        }
    }
    ...
    public function undelivered_product_print()
    {
        $undelivered_products = RequestProduct::with('products')
            ->select(DB::raw("product_id, (sum(qnty) - sum(del_qnt)) as undelivered_product"))
            ->where('qnty', '<>', 'del_qnt')
